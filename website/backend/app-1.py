...
replacements_dict = {
    ...
    "PERCENT": lambda: str(f"{random.randint(1, 100)}%"),                                               # Other - SpacyNER
    ...
    "STATE": lambda: str(fake.state()),                                                                 # Location - DeBERTA
    "COMPANY_NAME": lambda: str(fake.company()),                                                        # Personal - DeBERTA
    "JOBTYPE": lambda: str(fake.job()),                                                                 # Personal - DeBERTA
    "EMAIL": lambda: str(fake.email()),                                                                 # Personal - DeBERTA, Rule-based
    "PHONE_NUMBER": lambda: str(fake.phone_number()),                                                   # Personal - DeBERTA
    "CITY": lambda: str(fake.city()),                                                                   # Location - DeBERTA
    ...
    "CURRENCYSYMBOL": lambda: str(fake.currency_symbol()),                                              # Account - DeBERTA
    ...
    "SSN": lambda: str(fake.ssn()),                                                                     # Personal - PII
    "ZIPCODE": lambda: str(fake.zipcode()),                                                             # Location - PII
    "CREDITCARDNUMBER": lambda: str(fake.credit_card_number()),                                         # Account - PII
    "ACCOUNTNUMBER": lambda: str(fake.bban()),                                                          # Account - PII, Rule-based
...
def pii_detection(detection_type, text, user_id, multithreading, processor_type):
    ...
    if (multithreading == "yes"):
        ...
        # Remove overlapping
        final_replacement_list = find_non_overlapping_intervals(combined_replacement_list)
